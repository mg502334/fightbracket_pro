...
# Use -final.png to avoid all caching issues for the user
img_logo = Image.open(logo_src).convert("RGBA").resize((100, 100), Image.Resampling.LANCZOS)
img_logo.save(os.path.join(out_dir, "logo-100x100-final.png"), format="PNG")

img_icon = Image.open(logo_src).convert("RGBA").resize((24, 24), Image.Resampling.LANCZOS)
img_icon.save(os.path.join(out_dir, "icon-24x24-final.png"), format="PNG")

process_and_add_text_bubble(banner_src, os.path.join(out_dir, "discovery-300x200-final.png"), 300, 200, "FightBracket Pro", "ESPORTS", 0.75)
process_and_add_text_bubble(screenshot_src, os.path.join(out_dir, "screenshot-1024x768-final.png"), 1024, 768, "FightBracket Pro", "ESPORTS", 0.75)

print("Final bubble images generated!")
